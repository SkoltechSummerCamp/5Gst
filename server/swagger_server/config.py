...
# 
    ...
    def add(self, body):
        ...
        self.serverdict[(body.ip, body.port)] = (body.time, body.port_iperf)
        if self.log_file is not None:
            self.log_file.write(datetime.now().strftime('%X %x %Z') + ' add ' + body.ip + ':' + str(body.port) + ' ' + str(body.port_iperf) + ' ' + body.time.strftime('%X %x %Z') + '\n')
            self.log_file.flush()
    
    def get(self):
        body = self.serverdict.popitem()
        if self.log_file is not None:
            self.log_file.write(datetime.now().strftime('%X %x %Z') + ' send ' + body[0][0] + ':' + str(body[0][1]) + ' ' + str(body[1][1]) +  ' ' + body[1][0].strftime('%X %x %Z') + '\n')
            self.log_file.flush()
        return body
    ...
    def remove(self,body):
        if (body.ip, body.port) in self.serverdict:
            ...
            if self.log_file is not None:
                self.log_file.write(datetime.now().strftime('%X %x %Z') + ' del ' + body.ip + ':' + str(body.port) + ' ' + str(port_iperf) + ' '  + time.strftime('%X %x %Z') + '\n')
                self.log_file.flush()
            return time
